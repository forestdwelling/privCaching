...
def run_RLBD_continuous():
    ...
    for step in range(total_step):
        ...
        print('state:', state)
        print('action:', action)
        print('next_state:', next_state, 'trans_ratio:', generator.r)
        print('reward:', reward, calc_CHR(true_counts, calc_top_k(counts, k)))
        allocator.memory.push(state, action, next_state, reward)
        allocator.learn()
        state = next_state
        last_true_counts = true_counts
    print('Mean:', record.mean(0))
    torch.save(allocator.actor_target,'model/rlbd_continuous_actor.pt')
    torch.save(allocator.critic_target,'model/rlbd_continuous_critic.pt')

def run_RLBD_discrete():
    global counts
    global true_counts, last_true_counts
    allocator = DQNAgent(state_space_dim=10, action_space_dim=omega+1)
    state = np.zeros(10)
    for step in range(total_step):
        print(f'\n=========================================== STEP {step+1} ===========================================\n')
        # generate data
        ...
        budget = min(action*data_budget/omega, remain)
        window.update(budget)
        if budget!=0:
            data_randomizer.set_budget(budget)
            # collect data
            private_counts = data_randomizer.randomize_group(data)
            counts = data_randomizer.aggregate(private_counts)
            next_state = np.concatenate((state[2:], [len(np.intersect1d(calc_top_k(last_true_counts, k), calc_top_k(true_counts, k)))/k, window.remain()]))
            top_k = calc_top_k(counts, k)
        else:
            next_state = np.concatenate((state[2:], [len(np.intersect1d(calc_top_k(last_true_counts, k), calc_top_k(true_counts, k)))/k, window.remain()]))
        # record metrics
        ...
        # learn
        reward = calc_CHR(true_counts, calc_top_k(counts, k)) + (remain - action*epsilon/omega)/epsilon
        print('state:', state)
        print('action:', action*data_budget/omega, 'remain:', remain)
        print('next_state:', next_state)
        print('reward:', reward, calc_CHR(true_counts, calc_top_k(counts, k)), (remain - action*data_budget/omega)/epsilon)
        allocator.memory.push(state, action, next_state, reward)
        allocator.learn()
        state = next_state
        last_true_counts = true_counts
    print('Mean:', record.mean(0))
    torch.save(allocator.target_net,'model/rlbd_discrete.pt')

def run_RLBA():
    global counts
    global baseline
    baseline_count = 1
    allocator = DQNAgent(state_space_dim=2, action_space_dim=2)
    state = np.array([1, window.remain()])
    for step in range(total_step):
        print(f'\n=========================================== STEP {step+1} ===========================================\n')
        # generate data
        data = generator.generate()
        ...
        if budget!=0:
            ...
            next_state = np.array([mean_absolute_error(new_counts, counts), window.remain()])
            counts = new_counts
        ...
        elif action!=0 and remain==0:
            next_state = np.array([1, window.remain()])
            reward = -1
        elif action==0 and remain==epsilon:
            next_state = np.array([1, window.remain()])
            reward = -1
        else:
            next_state = np.array([1, window.remain()])
        ...
        # learn
        print('state:', state)
        print('action:', action, 'remain:', window.remain())
        print('next_state:', next_state)
        print('reward:', reward, calc_CHR(true_counts, calc_top_k(counts, k)), baseline)
        allocator.memory.push(state, action, next_state, reward)
        allocator.learn()
        state = next_state
    print('Mean:', record.mean(0))
    torch.save(allocator.target_net,'model/rlba.pkl')